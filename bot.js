...
bot.onText(/\/start/, (msg) => {
  ...
  const welcomeMessage = `Hello, ${username}!\n\n`
    ...
    + 'You can use this bot to shorten URLs using the botlatournament.xyz service.\n\n'
...
bot.onText(/\/api (.+)/, (msg, match) => {
  ...
  const response = `botlatournament API token set successfully.✅️✅️ Your token: ${userToken}`;
...
async function shortenUrlAndSend(chatId, Url) {
  // Retrieve the user's botlatournament API token from the database
  const arklinksToken = getUserToken(chatId);

  if (!arklinksToken) {
    bot.sendMessage(chatId, 'Please provide your botlatournament API token first. Use the command: /api YOUR_botlatournament_API_TOKEN');
    return;
  }

  try {
    const apiUrl = `https://botlatournament.xyz/api?api=${arklinksToken}&url=${Url}`;

    // Make a request to the https://botlatournament.xyz/api?api= API to shorten the URL
...
// Function to save user's botlatournament API token to the database (Replit JSON database)
...
// Function to retrieve user's botlatournament API token from the database
